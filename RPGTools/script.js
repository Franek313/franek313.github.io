// Zamiast używać var, korzystamy z const i let
let currentCategory = "";
let musicAudioPlayer;
let tempMusicAudioPlayer;
let currentMusicPlayer;
let globalVolume = 1.0;
const fadeDelay = 1500;

// Nowa struktura danych dla światów
const worlds = {
    Fantasy: {
        prefix: 'f_',
        style: {
...
// Przechowywanie JSON bezpośrednio jako obiekt
...
// Funkcja do odtwarzania losowej piosenki z danej kategorii
function playRandomSong(category) {
    const songNameElement = $("#songName");
    const songs = musicMap.get(category);
    let newSongName = '';
    let randomIndex = 0;

    do {
        randomIndex = Math.floor(Math.random() * songs.length);
        newSongName = songs[randomIndex].replace('.mp3', '');
        if (songs.length === 1) break;
    } while (songNameElement.text() === newSongName);

    songNameElement.text(newSongName);
    const audioUrl = `Audio/${category.replace(worldPrefix, "")}/${songs[randomIndex]}`;

    const nextPlayer = currentMusicPlayer === musicAudioPlayer ? tempMusicAudioPlayer : musicAudioPlayer;
    const currentPlayer = currentMusicPlayer;

    nextPlayer[0].src = audioUrl;
    nextPlayer[0].volume = 0;
    nextPlayer[0].play();
    nextPlayer.animate({
        volume: globalVolume
    }, fadeDelay);
...
// Funkcja generująca przyciski gatunków
function generateGenreButtons(worldName) {
    const genresPanel = $('#genresPanel');   

    console.log(musicMap.get(worldName).get("Action"))

    for (const key of musicMap.get(worldName).keys()){
        console.log(key)
        const index = key.indexOf('_');
        let worldPrefix = key.substring(0, index + 1)
        if (key.startsWith(worldPrefix)) {
            const genreName = key.replace(worldPrefix, "");
            const button = $('<button>')
                .text(genreName)
                .addClass('GenreButton')
                .css(worldsButtonsStyles[worldPrefix])
                .click(() => {
                    currentCategory = key;
...
// Funkcja generująca przyciski światów
function generateWorldButtons() {
    const worldsPanel = $('#worldsPanel');
    ...
    Object.keys(worlds).forEach((worldName) => {
        const button = $('<button>')
            ...
            .click(() => {
                worldPrefix = worlds[worldName].prefix;
                $('.GenreButton').remove();
                generateGenreButtons(worldName);
                $('#songName').css({
                    ...worlds[worldName].style,
    ...
    // Dodanie przycisku do wysuwania panelu
    const worldsButton = $('<button>').attr('id', 'worldsButton');
    const img = $('<img>').attr('src', 'worlds_button.png').attr('alt', 'Toggle Icon').css("width", "60px");
    worldsButton.append(img).click(() => {
        const left = worldsPanel.css('left') === '-250px' ? '0' : '-250px';
        worldsPanel.css('left', left);
    });
...
$(document).ready(() => {
    // Inicjalizacja mapy muzyki
    musicMap = new Map(Object.entries(folderStructureJSON));
    genres = Array.from(musicMap.keys());
    ...
    generateGenreButtons("Fantasy");

    // Obsługa suwaka głośności
    $("#volume-slider").on("input", function () {
        globalVolume = parseFloat($(this).val());
        musicAudioPlayer[0].volume = globalVolume;
        tempMusicAudioPlayer[0].volume = globalVolume;
    });
    ...
    musicAudioPlayer.on('ended', () => {
        playRandomSong(currentCategory);
    });
    tempMusicAudioPlayer.on('ended', () => {
        playRandomSong(currentCategory);
    });
});